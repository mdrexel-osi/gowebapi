...
func (a *AttributeTraitApiService) AttributeTraitGet(ctx context.Context, name string, localVarOptionals map[string]interface{}) (AttributeTrait, *http.Response, error) {
	var (
		...
		successPayload     AttributeTrait
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/attributetraits/{name}"
...
func (a *AttributeTraitApiService) AttributeTraitGetByCategory(ctx context.Context, category []string, localVarOptionals map[string]interface{}) (ItemsAttributeTrait, *http.Response, error) {
	var (
		...
		localVarFileName   string
		localVarFileBytes  []byte
		successPayload     ItemsAttributeTrait
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/attributetraits"
	...
	localVarQueryParams.Add("category", parameterToString(category, "multi"))
	if localVarTempParam, localVarOk := localVarOptionals["selectedFields"].(string); localVarOk {
		localVarQueryParams.Add("selectedFields", parameterToString(localVarTempParam, ""))
	}
	// to determine the Content-Type header
